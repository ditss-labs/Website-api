...
const setupModalForApi = (apiData) => {
    ...
    if (paramKeys.length > 0) {
        ...
        if (settings.apiSettings?.requireApikey) {
            ...
            // AUTO-FILL untuk API key
            const apikeyValue = paramsFromPath.get("apikey");
            if (apikeyValue) {
                apikeyInputField.value = decodeURIComponent(apikeyValue);
            }
        ...
        paramKeys.forEach((paramKey) => {
            if (paramKey === "apikey") return; // Skip jika sudah dibuat di atas
            ...
            // Set placeholder dengan contoh jika ada
            if (apiData.params && apiData.params[paramKey]) {
                let placeholder = `Enter ${paramKey}...`;
                if (typeof apiData.params[paramKey] === "object" && apiData.params[paramKey].example) {
                    placeholder = `Example: ${apiData.params[paramKey].example}`;
                }
            ...
            // AUTO-FILL: Set nilai dari parameter URL atau contoh
        ...
        // AUTO-FILL untuk API key jika ada di URL
        const paramsFromPath = new URLSearchParams(apiData.path.split("?")[1]);
        const apikeyValue = paramsFromPath.get("apikey");
        if (apikeyValue) {
            apikeyInputField.value = decodeURIComponent(apikeyValue);
